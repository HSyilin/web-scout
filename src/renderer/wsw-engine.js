/**
 * WSW 引擎 - 生成自包含的 .wsw 演示文件
 * .wsw 本质是一个 HTML 文件，浏览器可直接打开
 * 支持自由画布：拖拽、缩放、网格吸附、文字框、表格、形状等
 */

// ===== 引擎 CSS（压缩后嵌入） =====
const ENGINE_CSS = `*{margin:0;padding:0;box-sizing:border-box}
...
.insert-panel{position:fixed;top:60px;left:50%;transform:translateX(-50%);background:rgba(18,18,46,.96);backdrop-filter:blur(16px);border:1px solid #2a2a5a;border-radius:14px;padding:16px;z-index:1001;box-shadow:0 8px 32px rgba(0,0,0,.5);display:none;min-width:340px}
.insert-panel.show{display:block;animation:ctxIn .15s ease}
.insert-panel h3{font-size:13px;color:#e94560;margin-bottom:12px}
.insert-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:8px}
.insert-item{background:#0e0e24;border:1px solid #2a2a5a;border-radius:10px;padding:14px 8px;text-align:center;cursor:pointer;transition:all .15s;display:flex;flex-direction:column;align-items:center;gap:6px}
.insert-item:hover{background:#1a1a3e;border-color:#e94560;transform:translateY(-2px)}
.insert-item .ii-icon{font-size:22px}
.insert-item .ii-label{font-size:11px;color:#aaa}
.bg-panel{position:fixed;top:60px;right:16px;background:rgba(18,18,46,.96);backdrop-filter:blur(16px);border:1px solid #2a2a5a;border-radius:14px;padding:16px;z-index:1001;box-shadow:0 8px 32px rgba(0,0,0,.5);display:none;width:280px}
.bg-panel.show{display:block;animation:ctxIn .15s ease}
.bg-panel h3{font-size:13px;color:#e94560;margin-bottom:12px}
.bg-section{margin-bottom:14px}
.bg-section label{font-size:11px;color:#888;display:block;margin-bottom:6px}
.bg-color-row{display:flex;gap:6px;flex-wrap:wrap}
.bg-color-swatch{width:32px;height:32px;border-radius:8px;cursor:pointer;border:2px solid transparent;transition:border-color .15s}
.bg-color-swatch:hover,.bg-color-swatch.active{border-color:#fff}
.bg-gradient-row{display:flex;gap:6px;flex-wrap:wrap}
.bg-gradient-swatch{width:48px;height:32px;border-radius:8px;cursor:pointer;border:2px solid transparent;transition:border-color .15s}
.bg-gradient-swatch:hover,.bg-gradient-swatch.active{border-color:#fff}
.bg-input-row{display:flex;gap:6px;align-items:center}
.bg-input-row input[type="color"]{width:40px;height:32px;border:none;border-radius:6px;cursor:pointer;background:none}
.bg-input-row input[type="text"]{flex:1;background:#0e0e24;border:1px solid #2a2a5a;border-radius:6px;padding:6px 10px;color:#ccc;font-size:12px;outline:none}
.bg-input-row input[type="text"]:focus{border-color:#e94560}
.bg-upload-btn{background:#0e0e24;border:1px dashed #2a2a5a;border-radius:8px;padding:10px;text-align:center;cursor:pointer;font-size:12px;color:#888;transition:all .15s}
.bg-upload-btn:hover{border-color:#e94560;color:#e94560}
...
.context-menu.show{display:block;animation:ctxIn .12s ease}
...
.help-panel.show{display:block;animation:ctxIn .15s ease}
...
// ===== 引擎 JS（压缩后嵌入） =====
const ENGINE_JS = `(function(){'use strict';
const WSW_DATA=__WSW_DATA_PLACEHOLDER__;
document.getElementById('toolbarTitle').textContent=WSW_DATA.title;
const PRESET_COLORS=['#1a1a2e','#0d1b2a','#1b2838','#2d1b3d','#1a2e1a','#2e1a1a','#0a0a0a','#ffffff','#f5f5f0','#e8e0d4'];
const PRESET_GRADIENTS=['linear-gradient(135deg,#667eea,#764ba2)','linear-gradient(135deg,#f093fb,#f5576c)','linear-gradient(135deg,#4facfe,#00f2fe)','linear-gradient(135deg,#43e97b,#38f9d7)','linear-gradient(135deg,#fa709a,#fee140)','linear-gradient(135deg,#a18cd1,#fbc2eb)','linear-gradient(135deg,#ffecd2,#fcb69f)','linear-gradient(135deg,#0c0c1d,#1a1a3e)'];
...
function initBgPanel(){const colorRow=document.getElementById('bgColorRow');PRESET_COLORS.forEach(c=>{const sw=document.createElement('div');sw.className='bg-color-swatch'+(WSW_DATA.background.value===c?' active':'');sw.style.background=c;sw.onclick=()=>{WSW_DATA.background={type:'color',value:c};applyBackground();colorRow.querySelectorAll('.bg-color-swatch').forEach(s=>s.classList.remove('active'));sw.classList.add('active');document.getElementById('bgCustomColor').value=c;document.getElementById('bgCustomHex').value=c};colorRow.appendChild(sw)});const gradRow=document.getElementById('bgGradientRow');PRESET_GRADIENTS.forEach(g=>{const sw=document.createElement('div');sw.className='bg-gradient-swatch';sw.style.background=g;sw.onclick=()=>{WSW_DATA.background={type:'gradient',value:g};applyBackground();gradRow.querySelectorAll('.bg-gradient-swatch').forEach(s=>s.classList.remove('active'));sw.classList.add('active');colorRow.querySelectorAll('.bg-color-swatch').forEach(s=>s.classList.remove('active'))};gradRow.appendChild(sw)});document.getElementById('bgCustomColor').oninput=e=>{document.getElementById('bgCustomHex').value=e.target.value;WSW_DATA.background={type:'color',value:e.target.value};applyBackground();colorRow.querySelectorAll('.bg-color-swatch').forEach(s=>s.classList.remove('active'));gradRow.querySelectorAll('.bg-gradient-swatch').forEach(s=>s.classList.remove('active'))};document.getElementById('bgCustomHex').onchange=e=>{const v=e.target.value.trim();if(/^#[0-9a-fA-F]{6}$/.test(v)){document.getElementById('bgCustomColor').value=v;WSW_DATA.background={type:'color',value:v};applyBackground()}};document.getElementById('bgUploadBtn').onclick=()=>document.getElementById('bgFileInput').click();document.getElementById('bgFileInput').onchange=e=>{const file=e.target.files[0];if(!file)return;const reader=new FileReader();reader.onload=ev=>{WSW_DATA.background={type:'image',value:ev.target.result};applyBackground();colorRow.querySelectorAll('.bg-color-swatch').forEach(s=>s.classList.remove('active'));gradRow.querySelectorAll('.bg-gradient-swatch').forEach(s=>s.classList.remove('active'));showHint('背景图片已设置')};reader.readAsDataURL(file)};document.getElementById('btnToggleGrid').onclick=()=>{WSW_DATA.showGrid=!WSW_DATA.showGrid;applyBackground();showHint(WSW_DATA.showGrid?'网格已显示':'网格已隐藏')}}
...
function createElementByType(card){if(card.type==='textbox')return createTextBox(card);if(card.type==='table')return createTable(card);if(['rect','circle','triangle','line','arrow','star'].includes(card.type))return createShape(card);return createCardElement(card)}
...
document.addEventListener('mouseup',()=>{if(isPanning){isPanning=false;canvas.classList.remove('panning')}if(dragCard){const el=document.querySelector('[data-id="'+dragCard.id+'"]');if(el)el.classList.remove('dragging');clearSnapLines();dragCard=null}resizeCard=null});
...
document.getElementById('btnHelp').onclick=()=>{helpPanel.classList.toggle('show');insertPanel.classList.remove('show');bgPanel.classList.remove('show')};
document.getElementById('btnInsert').onclick=()=>{insertPanel.classList.toggle('show');bgPanel.classList.remove('show');helpPanel.classList.remove('show')};
document.getElementById('btnBg').onclick=()=>{bgPanel.classList.toggle('show');insertPanel.classList.remove('show');helpPanel.classList.remove('show')};
document.getElementById('btnSave').onclick=saveLayout;
document.getElementById('btnLoad').onclick=()=>document.getElementById('fileInput').click();
document.getElementById('fileInput').onchange=loadLayout;
function saveLayout(){const layout={version:'1.0',title:WSW_DATA.title,url:WSW_DATA.url,createdAt:WSW_DATA.createdAt,savedAt:new Date().toISOString(),background:WSW_DATA.background,showGrid:WSW_DATA.showGrid,cards:WSW_DATA.cards};const json=JSON.stringify(layout,null,2);const blob=new Blob([json],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=(WSW_DATA.title||'wsw-layout')+'.wsw';a.click();URL.revokeObjectURL(a.href);showHint('布局已保存为 .wsw 文件')}
function loadLayout(e){const file=e.target.files[0];if(!file)return;const reader=new FileReader();reader.onload=ev=>{try{const layout=JSON.parse(ev.target.result);if(!layout.cards||!Array.isArray(layout.cards))throw new Error('Invalid layout');WSW_DATA.title=layout.title||WSW_DATA.title;WSW_DATA.url=layout.url||WSW_DATA.url;WSW_DATA.createdAt=layout.createdAt||WSW_DATA.createdAt;WSW_DATA.background=layout.background||{type:'color',value:'#1a1a2e'};WSW_DATA.showGrid=layout.showGrid!==undefined?layout.showGrid:true;WSW_DATA.cards=layout.cards;document.getElementById('toolbarTitle').textContent=WSW_DATA.title;applyBackground();selectedCards.clear();renderCards();setTimeout(()=>{fitView();showHint('布局已导入：'+file.name)},100)}catch(err){showHint('导入失败：文件格式错误')}};reader.readAsText(file);e.target.value=''}
document.addEventListener('click',e=>{if(!e.target.closest('#helpPanel')&&!e.target.closest('#btnHelp'))helpPanel.classList.remove('show');if(!e.target.closest('#insertPanel')&&!e.target.closest('#btnInsert'))insertPanel.classList.remove('show');if(!e.target.closest('#bgPanel')&&!e.target.closest('#btnBg'))bgPanel.classList.remove('show')});
...
document.querySelectorAll('.insert-item').forEach(item=>{item.addEventListener('click',()=>{const type=item.dataset.type;insertPanel.classList.remove('show');const cw=canvas.clientWidth,ch=canvas.clientHeight;const cx=Math.round((-panX+cw/2)/zoom/SNAP_DIST)*SNAP_DIST;const cy=Math.round((-panY+ch/2)/zoom/SNAP_DIST)*SNAP_DIST;if(type==='textbox'){const card={id:Date.now(),type:'textbox',name:'文字框',content:'新文字框',x:cx-120,y:cy-30,w:240,h:60,fontSize:15,color:'#eee',bold:false,z:++maxZ};WSW_DATA.cards.push(card);cardsContainer.appendChild(createTextBox(card))}else if(type==='table'){const card={id:Date.now(),type:'table',name:'表格',rows:3,cols:3,data:[["列1","列2","列3"],["数据","数据","数据"],["数据","数据","数据"]],x:cx-160,y:cy-80,w:320,h:160,z:++maxZ};WSW_DATA.cards.push(card);cardsContainer.appendChild(createTable(card))}else{const colors=['#e94560','#4fc3f7','#81c784','#ffb74d','#ba68c8','#f06292'];const card={id:Date.now(),type:type,name:type,x:cx-60,y:cy-60,w:120,h:120,color:colors[Math.floor(Math.random()*colors.length)],fill:'rgba(255,255,255,0.05)',strokeWidth:2,z:++maxZ};WSW_DATA.cards.push(card);cardsContainer.appendChild(createShape(card))}saveUndoState();showHint('已插入 '+item.querySelector('.ii-label').textContent)})});
...
applyBackground();initBgPanel();renderCards();setTimeout(()=>{fitView();showHint('双击空白处创建文字框 · 点击 +插入 添加表格/形状 · 🎨背景 更换画布')},400)})();`;
...
<title>__TITLE_PLACEHOLDER__ - WSW</title>
<style>${ENGINE_CSS}</style>
</head>
<body>
<div class="toolbar">
  <span class="toolbar-title" id="toolbarTitle">__TITLE_PLACEHOLDER__</span>
  <div class="toolbar-sep"></div>
  <button class="toolbar-btn" id="btnSave" title="保存布局 (Ctrl+S)">💾 保存</button>
  <button class="toolbar-btn" id="btnLoad" title="导入布局">📂 导入</button>
  <input type="file" id="fileInput" accept=".wsw,.json" style="display:none">
  <div class="toolbar-sep"></div>
  <button class="toolbar-btn" id="btnInsert" title="插入">+ 插入</button>
  <button class="toolbar-btn" id="btnBg" title="背景设置">🎨 背景</button>
  <div class="toolbar-sep"></div>
...
<div class="insert-panel" id="insertPanel">
  ...
  <div class="insert-grid">
    <div class="insert-item" data-type="textbox"><span class="ii-icon">📝</span><span class="ii-label">文字框</span></div>
    <div class="insert-item" data-type="table"><span class="ii-icon">📊</span><span class="ii-label">表格</span></div>
    <div class="insert-item" data-type="rect"><span class="ii-icon"></span><span class="ii-label">矩形</span></div>
    <div class="insert-item" data-type="circle"><span class="ii-icon">⭕</span><span class="ii-label">圆形</span></div>
    <div class="insert-item" data-type="triangle"><span class="ii-icon">🔺</span><span class="ii-label">三角形</span></div>
    <div class="insert-item" data-type="line"><span class="ii-icon">➖</span><span class="ii-label">线条</span></div>
    <div class="insert-item" data-type="arrow"><span class="ii-icon">➡️</span><span class="ii-label">箭头</span></div>
    <div class="insert-item" data-type="star"><span class="ii-icon">⭐</span><span class="ii-label">星形</span></div>
  </div>
</div>
<div class="bg-panel" id="bgPanel">
  <h3>画布背景</h3>
  <div class="bg-section"><label>纯色背景</label><div class="bg-color-row" id="bgColorRow"></div></div>
  <div class="bg-section"><label>渐变背景</label><div class="bg-gradient-row" id="bgGradientRow"></div></div>
  <div class="bg-section"><label>自定义颜色</label><div class="bg-input-row"><input type="color" id="bgCustomColor" value="#1a1a2e"><input type="text" id="bgCustomHex" value="#1a1a2e" placeholder="#RRGGBB"></div></div>
  <div class="bg-section"><label>背景图片</label><div class="bg-upload-btn" id="bgUploadBtn">📁 点击选择图片</div><input type="file" id="bgFileInput" accept="image/*" style="display:none"></div>
  <div class="bg-section"><label>网格显示</label><div class="bg-input-row"><button class="toolbar-btn" id="btnToggleGrid" style="flex:1">切换网格</button></div></div>
</div>
...
<div class="help-panel" id="helpPanel">
  ...
  <div class="help-row"><span>新建文字框</span><span class="help-key">双击空白处</span></div>
  <div class="help-row"><span>保存布局</span><span class="help-key">Ctrl+S</span></div>
  <div class="help-row"><span>导入布局</span><span class="help-key">点击 导入</span></div>
 ...
 * @param {string} options.title - 演示标题
 * @param {string} options.url - 来源 URL
 * @param {Array} options.cards - 卡片数据（含 base64 资源）
 * @param {Object} [options.background] - 背景设置
 * @param {boolean} [options.showGrid] - 是否显示网格
 * @returns {string} 完整的 HTML 文件内容
 */
function generateWSW(options) {
  const {
    title = 'WebScout 演示',
    url = '',
    cards = [],
  ...
  // 替换占位符
  let html = ENGINE_HTML
    .replace(/__TITLE_PLACEHOLDER__/g, title.replace(/</g, '&lt;'))
    .replace('__WSW_DATA_PLACEHOLDER__', JSON.stringify(wswData));
 ...
 * 从应用资源数据构建 WSW 卡片
 * 自动布局：按类型排列，避免重叠
 * @param {Object} resources - { images, videos, audios, links, texts }
 * @param {Function} [getBase64] - 异步获取资源 base64 的函数
 * @returns {Promise<Array>} 卡片数组
 */
async function buildCardsFromResources(resources, getBase64) {
  const cards = [];
  let id = 1;
  const COL_W = 320;
  const ROW_H = 260;
  ...
  const startY = resources._url ? 110 : 80;

  // 处理各类资源
  const allResources = [];
  (resources.images || []).forEach(r => allResources.push({ ...r, resType: 'image' }));
  (resources.videos || []).forEach(r => allResources.push({ ...r, resType: 'video' }));
  (resources.audios || []).forEach(r => allResources.push({ ...r, resType: 'audio' }));
  (resources.links || []).forEach(r => allResources.push({ ...r, resType: 'link' }));
  (resources.texts || []).forEach(r => allResources.push({ ...r, resType: 'text' }));

  for (const res of allResources) {
    const x = START_X + col * (COL_W + GAP);
    const y = startY + row * (ROW_H + GAP);

    let card = {
      id: id++,
      type: res.resType,
      name: res.name || getFileName(res.url || ''),
      format: res.format || getExt(res.url || ''),
      x, y,
      w: COL_W,
      h: ROW_H,
      z: 1
    ...
    // 尝试获取 base64
    if (getBase64 && res.url) {
      try {
        const base64 = await getBase64(res.url, res.resType);
        if (base64) {
          if (res.resType === 'image') card.src = base64;
          else if (res.resType === 'video') card.poster = base64;
          else if (res.resType === 'audio') card.src = base64;
        }
      } catch (e) {
        // 获取失败，保留 URL
        card.url = res.url;
    ...
    // 文本内容
    if (res.resType === 'text' && res.content) {
      card.content = res.content.substring(0, 2000);
    }

    // 链接
    if (res.resType === 'link') {
      card.url = res.url || '';
      card.displayUrl = res.displayUrl || res.url || '';
    }

    // 如果没有 base64，保留 URL 作为降级
    if (!card.src && res.url && res.resType !== 'text' && res.resType !== 'link') {
      card.url = res.url;
    }
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { generateWSW, buildCardsFromResources };
}
